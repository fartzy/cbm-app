...
public class RMSRestCallProcessor extends AbstractProcessor<String, EDAMessage> {
	...
	public void process(String key, EDAMessage cbmMessage) {
		...
		try {
			...
			String sendMessage = cbmMessage.getMessage();
//			String sendMessage = "{\"records\":[{\"key\":\"ATLCP\", \"value\":{\"foo\":\"bar1\"}}]}";
		...
		if(restResponse.equalsIgnoreCase("ERROR") == false) {
			this.context.forward(key, cbmMessage.getPropertyCode() + " : " + edaMessage.getSequence().getPart2() + " --> SUCCESS", myConfig.getSuccessSinkName());
		} else {
			this.context.forward(key, cbmMessage.getPropertyCode() + " : " + edaMessage.getSequence().getPart2() + " --> ERROR", myConfig.getErrorSinkName());
		}
